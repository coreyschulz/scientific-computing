function [Q,fcount] = quadtx_alternate(F,a,b,tol,varargin)
...
% Recursive call 
[Q,k] = quadtxstep_alt(F, a, b, tol, fa, fc, fb, varargin{:});
fcount = k + 3;

% ---------------------------------------------------------

function [Q,fcount] = quadtxstep_alt(F,a,b,tol,fa,fc,fb,varargin)
...
if abs(Q2 - Q1) <= tol 
   ...
   [Qa,ka] = quadtxstep_alt(F, a, c, tol / 2, fa, fd, fc, varargin{:});
   [Qb,kb] = quadtxstep_alt(F, c, b, tol / 2, fc, fe, fb, varargin{:});
   Q  = Qa + Qb;
   fcount = ka + kb + 2;
end